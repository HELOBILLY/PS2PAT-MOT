...
class IOULoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None):
        ...
        g_w_intersect = torch.max(pred_l, target_l) + torch.max(pred_r, target_r)
        g_h_intersect = torch.max(pred_b, target_b) + torch.max(pred_t, target_t)
        ...
        if self.loc_loss_type == 'iou':
            ...
            target_w = (target_l + target_r) / 2.0
            target_h = (target_t + target_b) / 2.0
            pred_w = (pred_l + pred_r) / 2.0
            pred_h = (pred_t + pred_b) / 2.0

            w1 = torch.max(target_l, pred_l) - torch.min(target_l, pred_l)
            h1 = torch.max(target_t, pred_t) - torch.min(target_t, pred_t)

            xd = abs((pred_w/2.0) - (target_w/2.0)) + w1
            yd = abs((target_h/2.0) - (pred_h/2.0)) + h1

            d = (xd ** 2) + (yd ** 2) + 1e-7
            c = (g_w_intersect ** 2) + (g_h_intersect ** 2) + 1e-7
            u = d/c
            diou = ious - u
            losses = 1 - diou
        else:
...
class EMMLossComputation(object):
    def __init__(self, cfg):
        self.loss_type = cfg.MODEL.TRACK_HEAD.TRACKLOSS
        self.box_reg_loss_func = IOULoss(self.loss_type)
