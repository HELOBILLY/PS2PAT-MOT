...
def main(args, description="Initial ingestion"):
    ...
    for split_name, split_path in splits.items():
        ...
        for i, subdir in enumerate(subdirs):
            ...
            rel_base_dir = os.path.join(rel_base_dir, "img")
            print('rel_base_dir:', rel_base_dir)

            video_base_dir = os.path.join(vid_path, "img")
            all_imgs = [i for i in os.listdir(video_base_dir) if i.endswith('.jpg')]
            num_frames = len(all_imgs)
            img_file = video_base_dir + '/000001.jpg'
...
def write_data_split(args, dataset):
    ...
    process_dataset_splits(dataset, split_func, save=False)
