...
def build_data_filter_fn(dataset_key: str, *args, **kwargs):
    ...
    elif dataset_key == 'DETRAC':
        filter_fn = DETRACFilter(*args, **kwargs)
    elif dataset_key == 'SateMOT':
        filter_fn = SateMOTFilter(*args, **kwargs)
...
class SateMOTFilter(BaseFilter):
    """
        A class for filtering SateMOTFilter dataset entities during training and evaluation
        A gt entity will be filtered (ignored) if its confidence less than 0.1
        A predicted entity will be filtered (ignored) if it is matched to a ignored ground truth entity
        """
    def __init__(self, confidence_thresh=0.1, iou_thresh=0.1, is_train=False):
        self.confidence_thresh = confidence_thresh
        self.iou_thresh = iou_thresh

    def _filter(self, entity: AnnoEntity, ignored_gt_entities=None):
        if ignored_gt_entities is None:
            if entity.confidence < self.confidence_thresh:
                return True
        else:
            for entity_ in ignored_gt_entities:
                if bbs_iou(entity, entity_) >= self.iou_thresh:
...
class DETRACFilter(BaseFilter):
    """
        A class for filtering DETRACFilter dataset entities during training and evaluation
        A gt entity will be filtered (ignored) if its confidence less than 0.1
        A predicted entity will be filtered (ignored) if it is matched to a ignored ground truth entity
        """
    def __init__(self, confidence_thresh=0.1, iou_thresh=0.1, is_train=False):
